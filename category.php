...
if (isset($_COOKIE['cat_id'])) {
    $query = 'SELECT p.id, p.pro_name, p.pro_image, p.pro_price, c.cat_name, c.cat_desc FROM product as p LEFT JOIN category AS c ON p.pro_cat = c.id WHERE p.pro_cat = "'.$_COOKIE['cat_id'].'" ';
    $stmt = $dbc->query($query);
    $stmt_t = $dbc->query($query);
    $id = $_COOKIE['cat_id'];
    $row_t = $stmt_t->fetch();
}
?>
...
<head>
    ...
    <title><?php echo $row_t['cat_name'] ?></title>
    <style>
...
<header id="introduction">
    <div class="container p-5">
        <div class="row fill-viewport align-items-center">
            ...
            <h1 class="text-white-70"><?php echo $row_t['cat_name']?></h1>
            <p><?php echo $row_t['cat_desc']?></p>
            </div>
        </div>
    </div>
</header>
